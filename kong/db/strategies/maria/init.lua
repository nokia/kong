...
local encode_array                = mutils.encode_array
local decode_json                 = mutils.decode_json
...
do
  local convert_json = function(row_element)
    if row_element and row_element ~= null then
      return decode_json(row_element)
    end
  end


  local convert_json_array = function(row_element)
    if row_element and row_element ~= null then
      local array = decode_json(row_element)
      local results = {}
      for i in ipairs(array) do
        table.insert(results, decode_json(array[i]))
      end
      return results
    end
  end
  ...
  converters = {
    ...
    routes = {
      protocols = convert_json,
      hosts = convert_json,
      paths = convert_json,
      methods = convert_json,
      tags = convert_json,
      snis = convert_json,
      sources = convert_json_array,
      destinations = convert_json_array
    },
    plugins = {
      protocols = convert_json,
      config = convert_json,
    ...
    upstreams = {
      tags = convert_json,
      healthchecks = convert_json
    },
    targets = {
      tags = convert_json
    },
    oauth2_credentials = {
      redirect_uris = convert_json
    }
  }
end
...
local function escape_literal(connector, literal, field)
  ...
  if field then
    if field.timestamp then
      return concat { "FROM_UNIXTIME(", connector:escape_literal(tonumber(fmt("%.3f", literal))), ")" }
    end
    ...
    if field.type == "array" or field.type == "set" then
      ...
      if elements.timestamp then
        ...
        for i, v in ipairs(literal) do
          timestamps[i] = concat { "FROM_UNIXTIME(", connector:escape_literal(tonumber(fmt("%.3f", v))), ")" }
        end
...
local function toerror(strategy, err, primary_key, entity)
  ...
  --TODO Use Maria's error codes instead
  ...
  elseif find(err, "doesn't have a default value", 1, true) then
  ...
  elseif find(err, "foreign key constraint fails", 1, true) then
    log(NOTICE, err)
    if find(err, "child row", 1, true) then
      ...
      if not strategy.connector or
         not strategy.connector.config or
         not strategy.connector.config.database then
        return error("connector is broken")
      end
      local database = strategy.connector.config.database
      local parent_name = schema.name
      local child_name_regex = "`" .. database .. "`%.`([^`]+)`"
      local child_name = string.match(err, child_name_regex)
      return nil, errors:foreign_key_violation_restricted(parent_name,
                                                          tostring(child_name))
    end
  end
...
local function convert_to_table(row, schema)
  local converter = converters[schema.name]
  if converter then
    for element_name, convert in pairs(converter) do
      row[element_name] = convert(row[element_name], row)
    end
  end
  return row
end
...
local function convert_json_values(response, schema)
  for i in ipairs(response) do
    response[i] = convert_to_table(response[i], schema)
  end
  return response
end


local function convert_values(response, fields)
  if fields then
    for i, row in ipairs(response) do
      for col, value in pairs(row) do
        if fields[col].type == "boolean" then
          response[i][col] = (value == 1 and true) or false
        elseif fields[col].type == "integer" then
          response[i][col] = tonumber(value)
        end
      end
    end
  end
...
local function execute(strategy, statement_name, attributes, options)
  ...
  if not attributes then
    local res, err, err_code, sql_state = connector:query(statement)
    if res and res ~= null then
      res = convert_values(res, strategy.fields)
      res = convert_json_values(res, strategy.schema)
    end
    return res, err, err_code, sql_state
  end
  ...
  local res, err, err_code, sql_state = connector:query(sql)
  if res and res ~= null then
    res = convert_values(res, fields)
    res = convert_json_values(res, strategy.schema)
  end
...
function _mt:insert(entity, options)
  ...
  -- Simulate a Postgres RETURNING clause
  local primary_key = {}
  for _, key in ipairs(self.schema.primary_key) do
    primary_key[key] = entity[key]
  end
...
function _mt:update_by_field(field_name, unique_value, entity, options)
  ...
  -- Simulate a Postgres RETURNING clause
  local select_filter = {
    [field_name] = entity[field_name] or unique_value,
  }
  local res, err = execute(self, "select_by_" .. field_name, self.collapse(select_filter), options)
  if res then
    local row = res[1]
    if row then
      return self.expand(row), nil
...
function _mt:upsert(primary_key, entity, options)
  -- This method should UPDATE an existing entity (identified by PK) or
  -- INSERT a new entity if PK does not exist and unique parameters are unique.
  -- Otherwise it should fail. Especially it should return a 409 Conflict error
  -- when PK does not exist (or it is the same) and any unique parameter has
  -- already existed in the database;
  --
  -- INSERT ON DUPLICATE KEY does not give an expected result. If PK does not
  -- exists, it updates the entity with the specified unique parameter.
  -- Due to that following steps are executed:
  -- 1. INSERT. If no error, no more steps. If error, then go to a point 2.
  -- 2. If a duplicate error:
  -- 2a) If a duplicate PK (PRIMARY), then update the entity.
  -- 2b) If a duplicate unique parameter, then throw an error.
  local collapsed_entity = self.collapse(entity, primary_key)
  local res, err, err_code = execute(self, "insert", collapsed_entity, options)

  if err_code == 1062 then
    local regex = "Duplicate entry .+ for key 'PRIMARY'"
    -- @regex_engine
    if not find(err, regex) then
      return toerror(self, err, primary_key, entity)
    end
    res, err = execute(self, "upsert", collapsed_entity, options)
  end
...
function _mt:upsert_by_field(field_name, unique_value, entity, options)
  ...
  local collapsed_entity = self.collapse(entity, filter)
...
function _M.new(connector, schema, errors)
  ...
  for i = 1, fields_count do
    ...
    if not is_used_in_primary_key then
      ...
      upsert_expressions[update_fields_count] = name_escaped .. " = VALUES(" .. name_escaped .. ")"
    end
  ...
  if composite_cache_key then
    ...
    upsert_expressions[update_fields_count] = cache_key_escaped .. " = VALUES(" .. cache_key_escaped .. ")"
  ...
  if ttl then
    ...
    upsert_expressions[update_fields_count] = ttl_escaped .. " = VALUES(" .. ttl_escaped .. ")"
  ...
  if ttl then
    ...
    upsert_statement = concat {
      "INSERT INTO ",  table_name_escaped, " (", insert_columns, ")\n",
      "     VALUES (", insert_expressions, ")\n",
      "ON DUPLICATE KEY UPDATE ",  upsert_expressions, ";",
    }
    ...
    update_statement = concat {
      ...
      "      AND (", ttl_escaped, " IS NULL OR ", ttl_escaped, " >= NOW(6));",
    }
    ...
    page_first_statement = concat {
      "  SELECT ",  select_expressions, "\n",
      "    FROM ",  table_name_escaped, "\n",
      "   WHERE (", ttl_escaped, " IS NULL OR ", ttl_escaped, " >= NOW(6))\n",
      "ORDER BY ",  pk_escaped, "\n",
      "   LIMIT $1;";
    }
  ...
  local truncate_statement = concat {
    "CALL truncate_table(",
    connector:escape_literal(table_name), ", ",
    connector:escape_literal(database), "); "
  }
  ...
  if tags then
    ...
    for cond, op in pairs({["_and"] = "@>", ["_or"] = "&&"}) do
      if op == "@>" then
        where_statement = "JSON_CONTAINS(tags, $1)"
      else
        where_statement = "JSON_CONTAINS_COMMON_PART(tags, $1)"
      end
