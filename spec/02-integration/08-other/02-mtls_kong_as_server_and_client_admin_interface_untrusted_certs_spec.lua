...
os.execute("cp spec/02-integration/08-other/config/mtls_2.pem /opt/kong/busted/certs/")
os.execute("cp spec/02-integration/08-other/config/mtls_2.key /opt/kong/busted/certs/")
...
describe("Verify kong to upstream mTLS scenario", function()
    ...
    setup(function()
        ...
        assert(helpers.start_kong({
            ...
            kong_conf = "spec/02-integration/08-other/config/mtls_2.conf"
        }))
    ...
    it("Kong sends valid but untrusted client certificate - upstream configured with mTLS",
            function()
        local res = assert(proxy_client:send {
            method = "GET",
            path = "/myservice1Other"
        }
        )
